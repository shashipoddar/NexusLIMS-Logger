...
Cloning the NexusLIMS repository
++++++++++++++++++++++++++++++++
...
repository to your own account, or directly cloning the NexusLIMS repository to your local drive, shown below::
...
in the directory ``\GitHub\nexuslims\scripts\instruments.csv``.  Each row in the instruments.csv file
...
``computer_ip``, and ``computer_mount``.  More information on the Instrument table entries can be found
...
located in the NexusLIMS database prior to using the Logger GUI. More information on the Instruments table 
can be found `here <https://euclidtechlabs.github.io/nexuslims/database.html>`_. While some entries in the 
...
(located at ``\Github\nexuslims\scripts\create_db``) will input the instruments.csv data into a table contained 
